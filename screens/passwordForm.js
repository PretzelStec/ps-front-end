...
export function PasswordForm(){
    ...
    const [emailentry, setEmail] = React.useState(null)
    ...
    const submit = async () => {
        ...
        Axios.post('https://password-safe-backend.herokuapp.com/api/password/',
        {title:Encryption.cipher(title, key),url:Encryption.cipher(url, key),email:Encryption.cipher(emailentry, key),password:Encryption.cipher(passwordentry, key), section:section}, 
        {headers:config}
        )
        .then(resp => {
            setSuccess(1)
        ...
        <ScrollView style={style.body}>
            <View style={style.container}>
                ...
                <View style={style.FormContainer}>
                    ...
                    <Text style={style.labels}>URL</Text>
                    <TextInput style={style.inputs} onChangeText={(text)=>{setUrl(text)}} />
                    <Text style={style.labels}>Email</Text>
                    <TextInput style={style.inputs} onChangeText={(text)=>{setEmail(text)}} />
                    <Text style={style.labels}>Password</Text>
                    <TextInput style={style.inputs} onChangeText={(text)=>{setPassword(text)}} />
                    <Text style={style.labels}>Folder</Text>
                    <TextInput style={style.inputs} onChangeText={(text)=>{setSection(text)}} />
                </View>
